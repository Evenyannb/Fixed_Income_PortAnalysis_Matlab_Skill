...
## Pricing Method — Choose Based on Use Case

**Approximate pricing** (fast, used in morning DV01 aggregation):
- Even coupon periods, no day count, no accrued
- Fine for: DV01, duration, shock scenario P&L *changes*
- Not for: actual price levels, settlement, RV vs curve

**Full pricing** (use when PM asks about price, accrued, or RV):
- Use MATLAB Financial Toolbox `bndprice` / `bndconvy` if available
- Otherwise use the manual Actual/Actual implementation in
  references/modeling-patterns.md section 8
...
but note: "Prices are approximate — accrued interest not included."

## Caveats (state these once per session, not repeatedly)

- Morning DV01 aggregation uses approximate pricing (no accrued, no day count)
  — flag this clearly: "indicative only"
- For actual price/P&L: ask if they want full pricing with day count and accrued
